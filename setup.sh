...
function web_container() {
    ...
    sudo nsenter -t $PID1 -n ip link set dev eth-web1 address 00:00:00:12:01:80
    sudo nsenter -t $PID1 -n ip addr add 172.16.12.80/24 dev eth-web1
    sudo nsenter -t $PID1 -n ip -6 addr add 2a0b:4e07:c4:12::80/64 dev eth-web1
    sudo nsenter -t $PID1 -n ip link set eth-web1 up

    sudo nsenter -t $PID2 -n ip link set dev eth-web2 address 00:00:00:12:02:80
    sudo nsenter -t $PID2 -n ip addr add 172.16.12.80/24 dev eth-web2
    sudo nsenter -t $PID2 -n ip -6 addr add 2a0b:4e07:c4:12::80/64 dev eth-web2
    sudo nsenter -t $PID2 -n ip link set eth-web2 up

    sudo nsenter -t $PID1 -n ip route del default 
    sudo nsenter -t $PID1 -n ip route add default via 172.16.12.1
    sudo nsenter -t $PID1 -n ip -6 route add default via 2a0b:4e07:c4:12::1
    sudo nsenter -t $PID2 -n ip route del default 
    sudo nsenter -t $PID2 -n ip route add default via 172.16.12.1
    sudo nsenter -t $PID2 -n ip -6 route add default via 2a0b:4e07:c4:12::1
}
